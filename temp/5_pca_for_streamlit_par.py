...
def process_single_subgraph(
    ...
    # Set up CUDA for this process
    device = set_device()
    torch.set_grad_enabled(False)

    # Load model and SAE in this process
    ...
    fs_splitting_nodes = node_df[
        (node_df["subgraph_id"] == subgraph_id)
        & (node_df["subgraph_size"] == subgraph_size)
    ]["node_id"].tolist()
    ...
    try:
        ...
        if pca_df is None:
            logging.warning(f"Skipping subgraph {subgraph_id} due to PCA failure")
            return subgraph_id, None

        return subgraph_id, (results, pca_df)
    except Exception as e:
        logging.error(f"Error processing subgraph {subgraph_id}: {str(e)}")
        return subgraph_id, None
...
def main():
    ...
    # Add progress bar for SAE IDs
    with tqdm(sae_ids, desc="Processing SAE IDs") as sae_pbar:
        for sae_id in sae_pbar:
            ...
            # Save node_df path for workers
            node_df_path = pj(
                results_path, f"dataframes/node_info_df_{activation_threshold_safe}.csv"
            )
            node_df = pd.read_csv(node_df_path)

            # Add nested progress bar for subgraph sizes
            with tqdm(
                subgraph_sizes_to_plot, desc="Processing subgraph sizes"
            ) as size_pbar:
                for subgraph_size in size_pbar:
                    results_dict = {}
                    subgraphs_to_process = pd.Series(
                        node_df[node_df["subgraph_size"] == subgraph_size][
                            "subgraph_id"
                        ]
                    ).unique()
                    ...
                    process_func = partial(
                        process_single_subgraph,
                        node_df_path=node_df_path,
                    ...
                    # Process subgraphs in parallel with progress tracking
                    with Pool(processes=4) as pool:
                        for subgraph_id, result in tqdm(
                            pool.imap(process_func, subgraphs_to_process),
                            total=len(subgraphs_to_process),
                            desc=f"Processing subgraphs of size {subgraph_size}",
                            leave=False,  # Don't leave inner progress bar
                        ):
                            if result is not None:
                                results_dict[subgraph_id] = result
                    ...
                    save_results_to_hdf5(
                        output_file, results_dict, save_options=save_options
                    )
                    ...
                    size_pbar.set_postfix({"Current Size": subgraph_size})

            sae_pbar.set_postfix({"Current SAE": sae_id})
