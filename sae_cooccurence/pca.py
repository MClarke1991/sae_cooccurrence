...
from PIBBSS.graph_generation import load_subgraph, plot_subgraph_static
...
def process_examples(
    ...
    for _ in pbar:
        ...
        max_feature_in_graph = torch.zeros_like(max_feature_indices, dtype=torch.float32)
        for i, idx in enumerate(max_feature_indices):
            max_feature_in_graph[i] = float(idx in feature_list_tensor)
        max_feature_info = torch.stack([max_feature_values, max_feature_indices.float(), max_feature_in_graph], dim=1)
...
def perform_pca_on_results(results: ProcessedExamples, n_components: int = 3):
    ...
    pca_df = pd.DataFrame(
        pca_embedding, columns=[f"PC{i+1}" for i in range(n_components)]
    )
...
def generate_data(
    model, sae, activation_store, fs_splitting_nodes, n_batches_reconstruction, decoder=False
):
        ...
        for _, row in df.iterrows():
            fig.add_trace(
                go.Bar(
                    x=[row["Feature Index"]],
                    y=[row["Activation"]],
                    marker_color=color_map[row["Is Feature Splitting"]],
                    showlegend=False,
                ),
                row=i + 2,
                col=1,
...
def create_bar_plot_specific(
    ...
    # Sort the dataframe
    if plot_only_fs_nodes:
        plot_df = plot_df.sort_values("Feature Index")
    else:
        plot_df = plot_df.sort_values(
...
def analyze_specific_points_animated(
    ...
    for point_id in point_ids:
        frame_data, context = create_frame_data(
            results, fs_splitting_nodes, fs_splitting_cluster,
            activation_threshold, node_df, results_path, pca_df,
            point_id, plot_only_fs_nodes, fixed_pos, global_max_activation
        )
        frame = go.Frame(
            data=frame_data, 
            name=str(point_id),
    ...
    fig.update_layout(
        ...
        sliders=[{
            ...
            "steps": [
                {
                    "args": [[str(point_id)], {"frame": {"duration": 300, "redraw": True},
                                               "mode": "immediate",
        ...
        yaxis2=dict(range=[0, global_max_activation])  # Set fixed y-axis range for bar plot
    )
...
def create_frame_data(
    ...
    pca_trace = go.Scatter(
        ...
        text=[context if idx == point_id else None for idx, context in zip(pca_df.index, pca_df['context'])],
        hoverinfo='text',
        showlegend=False,
        xaxis='x',
        yaxis='y'
    ...
    if plot_only_fs_nodes:
        ...
        df = df.sort_values('Feature Index')
        df['Feature Index'] = df['Feature Index'].astype(str)
    ...
    bar_trace = go.Bar(
        ...
        y=df['Activation'],
        marker_color=['red' if idx == fs_splitting_cluster else 'blue' for idx in df['subgraph_id']],
        showlegend=False,
        xaxis='x2',
        yaxis='y2'
    )
    ...
    edge_trace, node_trace = create_subgraph_traces(subgraph, node_df, activation_array, fixed_pos)
...
def create_subgraph_traces(subgraph, node_df, activation_array, pos):
    ...
    normalized_activations = (node_activations - np.min(node_activations)) / (np.max(node_activations) - np.min(node_activations))
    ...
    for node in subgraph.nodes():
        node_info = node_df[node_df['node_id'] == node].iloc[0]
        top_tokens = ast.literal_eval(node_info['top_10_tokens'])
        node_hover_text.append(f"Feature: {node}<br>Activation: {activation_array[node]:.4f}<br>Top token: {top_tokens[0]}")
