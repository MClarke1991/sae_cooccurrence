...
def evaluate_probe(
    probe: nn.Module, test_loader: DataLoader, device: torch.device
) -> dict[str, float]:
...
def plot_metrics(metrics_history: dict, loss_history: dict, out_dir: str) -> None:
    ...
    for split in metrics_history:
        for metric in metrics_history[split][0].keys():
            ...
            ax1.plot(
                range(1, len(values) + 1), values, label=f"{split}_{metric}", marker="o"
            )
    ...
    for split, losses in loss_history.items():
        ax2.plot(range(1, len(losses) + 1), losses, label=f"{split}_loss", marker="o")
...
def train_probe(
    ...
    examples, labels = generate_balanced_examples(n_samples, config=config, seed=42)
...
def find_similar_sae_features(probe: nn.Module, sae: SAE, out_dir: str, top_k=10):
    ...
    plt.bar(
        range(top_k),
        top_similarities.detach().cpu().numpy(),
        tick_label=top_indices.cpu().numpy(),
    )
