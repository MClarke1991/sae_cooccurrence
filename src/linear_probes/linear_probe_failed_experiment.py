...
class TextGenerationConfig:
    ...
    digit_prob: float = (
        0.7  # Probability of using digit vs other words within negative class
    )
    min_number: int = 1
    max_number: int = 11
...
def validate_examples(examples: list[str], labels: list[int]) -> bool:
    ...
    if positive_count != len(labels) // 2:
        print(
            f"Error: Unbalanced classes. Positive examples: {positive_count}, "
...
class LinearProbe(nn.Module):
    def __init__(self, input_size):
        super().__init__()
        # Add more regularization and normalization
        ...
        nn.init.xavier_normal_(self.fc2.weight, gain=0.01)
...
def evaluate_probe(probe: nn.Module, test_loader: DataLoader):
    ...
    for threshold in thresholds:
        preds = (all_logits >= threshold).astype(np.int64)
    ...
    all_preds = (all_logits >= best_threshold).astype(np.int64)
    ...
    precision, recall, f1, _ = precision_recall_fscore_support(
        all_labels, all_preds, average="binary", zero_division="warn"
    )
...
def plot_metrics(metrics_history: dict, loss_history: dict, out_dir: str) -> None:
    """Plot training metrics and loss over epochs with separate panels for each metric type"""
    # Get all metric types (excluding threshold)
    metric_types = [m for m in metrics_history["train"][0].keys() if m != "threshold"]
    n_metrics = len(metric_types) + 1  # +1 for loss plot

    fig, axes = plt.subplots(n_metrics, 1, figsize=(12, 4 * n_metrics))

    # Plot loss
    for split, losses in loss_history.items():
        axes[0].plot(range(1, len(losses) + 1), losses, label=f"{split}", marker="o")
...
def train_probe(
    ...
    learning_rate=0.00001,  # Much smaller learning rate
    weight_decay=0.0001,
):
    ...
    # Use a more balanced criterion
    pos_weight = torch.tensor([1.0]).to(device)  # Start with balanced weight
    ...
    # Add validation metrics tracking with longer patience
    ...
    for epoch in tqdm(range(n_epochs), desc="Training"):
        ...
        for batch_activations, batch_labels in train_loader:
            ...
            # Calculate current batch statistics for adaptive loss weighting
            batch_pos = batch_labels.mean().item()
            if batch_pos > 0:  # Avoid division by zero
                pos_weight.data = torch.tensor([(1 - batch_pos) / batch_pos]).to(device)

            loss = criterion(logits, batch_labels)
            loss.backward()

            # More aggressive gradient clipping
            torch.nn.utils.clip_grad_norm_(probe.parameters(), max_norm=0.5)
...
def analyze_neurons(probe, n_top=10):
    ...
    weights = probe.fc2.weight.detach().cpu().numpy().squeeze()
    top_neurons = np.argsort(np.abs(weights))[-n_top:]

    # Save top neurons to CSV
    neuron_data = {"Neuron Index": top_neurons, "Weight": weights[top_neurons]}
    pd.DataFrame(neuron_data).to_csv(
        os.path.join(out_dir, f"top_neurons_n_examples_{n_examples}.csv"), index=False
    )
...
def find_similar_sae_features(probe: nn.Module, sae: SAE, out_dir: str, top_k=10):
    ...
    probe_weights = probe.fc2.weight.detach().squeeze()
    ...
    # Reshape SAE weights to 2D if needed
    if len(sae_weights.shape) > 2:
        sae_weights = sae_weights.reshape(sae_weights.shape[0], -1)
    ...
    # We need to get the probe weights to match the second dimension of SAE weights
    if probe_weights.shape[0] != sae_weights.shape[1]:
        print(
            "Warning: Probe weights dimension doesn't match SAE weights. Using first 384 dimensions of SAE weights."
        )
        sae_weights_norm = sae_weights_norm[:, : probe_weights.shape[0]]

    # Compute cosine similarities
    similarities = torch.matmul(
        sae_weights_norm, probe_weights_norm
    )  # Shape will be [24576]
    ...
    pd.DataFrame(feature_data).to_csv(
        os.path.join(out_dir, f"top_similar_sae_features_n_examples_{n_examples}.csv"),
        index=False,
    )

    # Plot the top similar features
    plt.figure(figsize=(12, 6))
    plt.bar(range(len(top_similarities)), top_similarities.detach().cpu().numpy())
    plt.title("Top Similar SAE Features to Linear Probe Weights")
    plt.xlabel("SAE Feature Index")
    plt.ylabel("Cosine Similarity")
    plt.xticks(range(len(top_similarities)), top_indices.cpu().numpy(), rotation=45)  # type: ignore
    plt.tight_layout()
    plt.savefig(
        os.path.join(out_dir, f"top_similar_sae_features_n_examples_{n_examples}.png")
    )
    plt.close()

    return top_indices.tolist(), top_similarities.detach().cpu().tolist()
...
if __name__ == "__main__":
    ...
    config_path = os.path.join(
        get_git_root(), "src", "linear_probes", "config_linear_gpt2_counting.toml"
    )
