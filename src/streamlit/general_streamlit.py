...
import pandas as pd
import streamlit as st
from PIBBSS.utils.saving_loading import load_npz_files
from PIBBSS.utils.set_paths import get_git_root
...
def load_subgraph_data(file_path, subgraph_id):
    with h5py.File(file_path, 'r') as f:
        ...
        # Load results
        results = {
            'all_fired_tokens': decode_if_bytes(load_dataset(group['all_fired_tokens'])),
            'all_reconstructions': load_dataset(group['all_reconstructions']),
            'all_graph_feature_acts': load_dataset(group['all_graph_feature_acts']),
            # 'all_feature_acts': load_dataset(group['all_feature_acts']),
            'all_max_feature_info': load_dataset(group['all_max_feature_info']),
            'all_examples_found': load_dataset(group['all_examples_found'])
        }
        
        # Load all_token_dfs
        token_dfs_group = group['all_token_dfs']
        all_token_dfs_data = {}
        for column in token_dfs_group.keys():
            all_token_dfs_data[column] = decode_if_bytes(load_dataset(token_dfs_group[column]))
...
def plot_pca_2d(pca_df, max_feature_info, fs_splitting_nodes):
    ...
    colors = ['grey' if not in_graph else color_map.get(feature, 'grey') 
    ...
    fig = px.scatter(pca_df, x='PC2', y='PC3',
    ...
    fig.update_layout(height=600, width=800,
                      ...
                      xaxis_title='PC2',
                      yaxis_title='PC3',
                      showlegend=False)  # Hide the legend in the main plot
...
def plot_feature_activations(all_graph_feature_acts, point_index, fs_splitting_nodes, context):
...
def load_available_subgraphs(file_path):
    with h5py.File(file_path, 'r') as f:
        return sorted([int(key.split('_')[1]) for key in f.keys() if key.startswith('subgraph_')])
def main():
    st.set_page_config(layout="wide")
    st.title('PCA Visualization with Feature Activations')

    git_root = get_git_root()
    results_root = pj(git_root, "results/cooc/gpt2-small/res-jb-feature-splitting/blocks_8_hook_resid_pre_24576/")
    pca_results_path = pj(results_root, "pca_for_streamlit", "graph_analysis_results_size_51.h5")

    # Load available subgraphs
    available_subgraphs = load_available_subgraphs(pca_results_path)

    # Dropdown for subgraph selection
    selected_subgraph = st.selectbox("Select a subgraph", available_subgraphs, key='subgraph_selector')

    activation_threshold = 1.5
    activation_threshold_safe = str(activation_threshold).replace(".", "_")
    node_df = pd.read_csv(pj(results_root, f"dataframes/node_info_df_{activation_threshold_safe}.csv"))
    fs_splitting_nodes = node_df.query('subgraph_id == @selected_subgraph')['node_id'].tolist()
    ...
    with col1:
        ...
        st.write("Color represents character count. Click on a point to see its feature activations.")
        ...
        selected_points = spe.plotly_events(pca_plot, click_event=True, override_height=600, key=f'pca_plot_{selected_subgraph}')
    ...
    with col3:
        ...
        if selected_points:
            ...
            matching_points = pca_df[(pca_df['PC2'] == selected_x) & (pca_df['PC3'] == selected_y)]
            ...
            if not matching_points.empty:
                ...
                feature_plot = plot_feature_activations(results['all_graph_feature_acts'], point_index, fs_splitting_nodes, context)
                feature_plot_placeholder.plotly_chart(feature_plot, use_container_width=True)
            else:
                st.write("The selected point is not in the current dataset. Please select a new point.")
        else:
