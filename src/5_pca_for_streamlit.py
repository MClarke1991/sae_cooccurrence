...
from PIBBSS.utils.saving_loading import set_device, load_model_and_sae
from PIBBSS.utils.set_paths import get_git_root
from PIBBSS.pca import process_examples, perform_pca_on_results

def process_graph_for_pca(model, sae, activation_store, fs_splitting_nodes, n_batches_reconstruction):
...
def save_results_to_hdf5(file_path, results_dict, save_all_feature_acts=True):
    with h5py.File(file_path, 'w') as f:
        for subgraph_id, (results, pca_df) in results_dict.items():
            ...
            # Save results
            group.create_dataset('all_fired_tokens', data=np.array([token.encode('utf-8') for token in results.all_fired_tokens]))
            group.create_dataset('all_reconstructions', data=results.all_reconstructions.cpu().numpy())
            group.create_dataset('all_graph_feature_acts', data=results.all_graph_feature_acts.cpu().numpy())
            if save_all_feature_acts:
                group.create_dataset('all_feature_acts', data=results.all_feature_acts.cpu().numpy())
            group.create_dataset('all_max_feature_info', data=results.all_max_feature_info.cpu().numpy())
            ...
            for column in results.all_token_dfs.columns:
                if results.all_token_dfs[column].dtype == 'object':
                    ...
                    data = np.array([str(item).encode('utf-8') for item in results.all_token_dfs[column].values])
                    token_dfs_group.create_dataset(column, data=data)
                else:
                    # For numeric dtypes, save directly
                    token_dfs_group.create_dataset(column, data=results.all_token_dfs[column].values)
            ...
            for column in pca_df.columns:
                if pca_df[column].dtype == 'object':
                    # For object dtype (likely strings), encode to UTF-8
                    data = np.array([str(item).encode('utf-8') for item in pca_df[column].values])
...
def main():
    ...
    for subgraph_size in subgraph_sizes_to_plot:
        results_dict = {}
        subgraphs_to_process = node_df[node_df['subgraph_size'] == subgraph_size]['subgraph_id'].unique()

        for subgraph_id in tqdm(subgraphs_to_process, desc=f"Processing subgraphs of size {subgraph_size}"):
            fs_splitting_nodes = node_df[node_df['subgraph_id'] == subgraph_id]['node_id'].tolist()
            results, pca_df = process_graph_for_pca(model, sae, activation_store, fs_splitting_nodes, n_batches_reconstruction)
        ...
        save_results_to_hdf5(output_file, results_dict, save_all_feature_acts=save_all_feature_acts)

        logging.info(f"Analysis completed for subgraph size {subgraph_size}. Results saved to {output_file}")
