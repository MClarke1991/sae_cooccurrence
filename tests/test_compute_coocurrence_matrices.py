...
def expected_results(mock_environment):
    ...
    expected_results_path = os.path.join(
        os.path.dirname(__file__), "expected_cooccurrence_matrices.pkl"
    )
    ...
    if not os.path.exists(expected_results_path):
        ...
        results = compute_cooccurrence_matrices(
...
def test_compute_cooccurrence_matrices(mock_environment, expected_results):
    ...
    for threshold in actual_results.keys():
        assert torch.allclose(
            actual_results[threshold], expected_results[threshold], rtol=1e-5, atol=1e-8
        )
