...
def mock_feature_acts_totals():
    ...
    return {
        0.1: torch.rand(100, 100),
        0.5: torch.rand(100, 100)
    }

@pytest.fixture
def expected_jaccard_results(mock_feature_acts_totals):
    # Define the path for our expected results file
    expected_results_path = os.path.join(os.path.dirname(__file__), "expected_jaccard_matrices.pkl")

    if not os.path.exists(expected_results_path):
        # If the file doesn't exist, compute the results and save them
        results = calculate_jaccard_matrices_chunked(mock_feature_acts_totals, device="cpu", chunk_size=10)
...
def test_calculate_jaccard_matrices_chunkedv3(mock_feature_acts_totals, expected_jaccard_results):
    # Suppress warnings for this test
    warnings.simplefilter("ignore")
    
    # Compute actual results
    actual_results = calculate_jaccard_matrices_chunked(mock_feature_acts_totals, device="cpu", chunk_size=10)

    # Compare results
    assert set(actual_results.keys()) == set(expected_jaccard_results.keys())
    for threshold in actual_results.keys():
        assert torch.allclose(actual_results[threshold], expected_jaccard_results[threshold], rtol=1e-5, atol=1e-8)
